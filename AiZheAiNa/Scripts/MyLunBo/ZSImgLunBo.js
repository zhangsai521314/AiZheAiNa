...
(function ($, window) {
    $.fn.ZSImgLunBo = function (options) {
        ...
        $.each(jsonData.data, function (index, item) {
            //加入图片数据
            if (item.herf == "") {
                item.herf = "javascript:void(0)";
            }
            $(xdivZSLunBoImg).append($("<a href='" + item.herf + "'><img src='" + item.src + "'></a>"));
        });
        //轮播标识加入到dom中
        ...
        if (op.isLunShowBoAnNiu) {
            ...
            $(xdivZSLunBoAnNiuIndivLunBoImg + " div").each(function (index, item) {
                ...
                if (index == 0) {
                    //后退
                    $i.click(function () {
                        LunBoAnNiuClick("down", false);
                    });
                } else {
                    //前进
                    $i.click(function () {
                        LunBoAnNiuClick("up", true);
                    });
                };
                if (op.isOpenZiDongLunBo) {
        ...
        function LunBoAnNiuClick(direction, isZhuCeMouseLunBoBiaoShi) {
            ...
            switch (op.LunBoFangShi.toLowerCase()) {
                case "1":
                    break;
                default:
                    DefaultClickLunBoAnNiuShowImg();
            }
            //注册轮播标识的mouse，只需注册一次
            if (isZhuCeMouseLunBoBiaoShi) {
                $(xdivZSLunBoBiaoShiIndivLunBoImg + " ul li").each(function () {
        ...
        //默认轮播方式
        function DefaultZiDongLunBo(op) {
            daTuZiDongLunBoID = setInterval(function () {
                LunBoAnNiuClick("up", false);
                DefaultClickLunBoAnNiuShowImg();
            }, op.ZiDongLunBoMiao);
        };
        //默认点击轮播按钮图片怎么出现
        function DefaultClickLunBoAnNiuShowImg() {
