...
(function ($, window) {
    ...
    function WebUpLoaderFile(c) {
        ...
        uploader = WebUploader.create({
            ...
            //只允许上传附件类型
            accept: {
                extensions: 'pdf,doc,xls,ppt,docx,xlsx,pptx,txt,zip,rar'
            },
        ...
        uploader.on('error', function (handler) {
            switch (handler) {
                case "Q_EXCEED_NUM_LIMIT":
                    ...
                    break;
                case "Q_TYPE_DENIED":
                    alert("pdf,doc,xls,ppt,docx,xlsx,pptx,txt,zip,rar 格式的文件");
                    break;
                case "F_EXCEED_SIZE":
                    alert("附件大小超出，附件最大" + c.fileSingleSizeLimit + "M");
                    break;
            }
        });
        ...
        //    var errArray = ["exe", "cs", "aspx", "html", "js", "css", "ashx", "ascx", "aspx", "htm", "sln", "suo", "dll", "csproj", "pdb", "cache", "db", "config"];
        ...
        uploader.on('fileQueued', function (file) {
            $list.append('<div id="' + file.id + '" class="item">' +
                ...
                '<div class="download" style="display:none;"><a class="adownload" style="width:20px;display:inline-block;">&nbsp</a></div>' +
                '<div class="del"></div>' +
            '</div>');
        });
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            if (c.showDownload) {
                target.find('#' + file.id).find('div.download:first').show();
                if (c.serverUrl.toLocaleLowerCase().indexOf("?") > 0) {
                    target.find('#' + file.id).find('div.download:first a:first').attr("href", c.serverUrl + "&operationType=down&FilePath=" + response.filePath);
                } else {
                    target.find('#' + file.id).find('div.download:first a:first').attr("href", c.serverUrl + "?operationType=down&FilePath=" + response.filePath);
                }
            }
        ...
        $list.on("click", ".del", function () {
            ...
            if ($ele.prevAll("div.state").eq(0).html() != "上传出错") {
                if (num >= 88888) {
                    ...
                    $.post(c.serverUrl, { operationType: "delete", imagePath: msrc }, function (data) {
                    });
                }
            };
        });

        $list.on("click", ".adownload", function () {
            ...
            $.ajax({
                ...
                type: "get",
                url: c.serverUrl,
                data: { operationType: "checkfile", FilePath: $(this).parent().parent().attr("msrc") },
                dataType: "json",
    ...
    function LoadingFile(c) {
        if (c.fielePathHiddenId != "" && c.filePathAttribute != "") {
            ...
            var downFielClass = "", downHref = "";
            if (!c.showDownload) {
                downFielClass = " style='display:none' ";
            }
            if (c.serverUrl.toLocaleLowerCase().indexOf("?") > 0) {
                downHref = "&";
            } else {
            ...
            $.each(mfileJsonData, function (i, em) {
                if (i == c.filePathAttribute) {
                    $.each(em, function (index, item) {
                        var num = FileNum();
                        $("#" + c.fileContainerID + " div.thelist").append($("<div class='item oldFile' msrc='" + item.filePath + "' id='WU_FILE_" + num + "'><div class='info'>" + item.filePath.split('/')[item.filePath.split('/').length - 1] + "</div><div class='state'>已上传</div><div  class='download' " + downFielClass + "><a class='adownload' href='" + c.serverUrl + downHref + "operationType=down&FilePath=" + item.filePath + "' style='width:20px;display:inline-block;'>&nbsp</a></div><div class='del'></div><span class='progress' style='display: none;'><span class='percentage'><span class='text'>100%</span><span style='width: 100%;' role='progressbar' class='bar'></span></span></span></div>"));
                    });
                    return false;
                }
            });
